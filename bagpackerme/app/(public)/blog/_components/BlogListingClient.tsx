      ...
      {isDefaultView && currentPage === 1 && featuredPost && (
        <section className="w-full bg-[#F7F9FA] pt-[clamp(40px,5vw,64px)] pb-[clamp(32px,4vw,48px)] px-mobile md:px-desktop relative z-10">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-[40px] items-center bg-white rounded-3xl p-6 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 transition-all duration-500 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] group/feature">
              {/* Left (55%) */}
              <div className="w-full lg:w-[55%] relative overflow-hidden rounded-2xl">
                <Link href={`/blog/${featuredPost.slug}`}>
                  <div className="absolute top-[16px] left-[16px] z-10 bg-white/90 backdrop-blur-sm text-teal px-[16px] py-[6px] rounded-full font-display text-[12px] font-bold uppercase tracking-widest leading-none shadow-sm">
                    {featuredPost.category}
                  </div>
                  <div className="aspect-[16/9] lg:aspect-[4/3] relative w-full overflow-hidden rounded-2xl">
                    <Image
              ...
              {/* Right (45%) */}
              <div className="w-full lg:w-[45%] flex flex-col items-start lg:pl-4">
                <span className="font-display text-[11px] font-bold tracking-[0.2em] text-teal/70 uppercase mb-[16px] opacity-80">
                  Featured Story
                </span>
                <Link href={`/blog/${featuredPost.slug}`} className="block group/title">
                  <h2 className="font-display text-[clamp(1.75rem,2.5vw,2.5rem)] text-void font-bold leading-[1.15] mb-[20px] group-hover/title:text-teal transition-colors duration-300 [text-wrap:balance]">
                    {featuredPost.title}
                  </h2>
                </Link>
                ...
                <div className="flex items-center gap-4 mb-[32px] w-full">
                  <div className="w-10 h-10 rounded-full bg-ice flex items-center justify-center text-teal font-display font-bold text-sm">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="font-body text-[13px] text-gray-400">
                      ...
                      {featuredPost.readTimeMinutes && <span className="mx-2">•</span>}
                      {featuredPost.readTimeMinutes && <span>{featuredPost.readTimeMinutes} min read</span>}
                    </span>
                  </div>
                </div>
                <Link href={`/blog/${featuredPost.slug}`}>
                  <button className="bg-void text-white font-display font-medium text-[13px] tracking-wide px-[32px] py-[14px] rounded-full transition-all duration-300 hover:bg-teal hover:shadow-[0_8px_20px_rgba(40,80,86,0.2)] hover:-translate-y-0.5 flex items-center gap-2">
                    Read Story
      ...
      <section className="w-full bg-[#F7F9FA] pb-[clamp(64px,10vw,120px)] pt-8 px-mobile md:px-desktop relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-[48px] lg:gap-[64px]">
          ...
          <div className="w-full lg:w-[66.666%]">
            ...
            {gridPosts.length === 0 ? (
               <div className="text-center py-24 bg-white/50 backdrop-blur-sm rounded-3xl border border-dashed border-gray-200 flex flex-col items-center justify-center">
               ...
               <>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-[64px]">
                   {currentGridPosts.map(post => (
                     <Link href={`/blog/${post.slug}`} key={post.id} className="group flex flex-col bg-white rounded-3xl overflow-hidden border border-gray-100/60 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_12px_30px_rgb(0,0,0,0.06)] hover:-translate-y-1 transition-all duration-400">
                        <div className="relative aspect-[4/3] w-full overflow-hidden">
                          ...
                          />
                          <div className="absolute top-[16px] left-[16px] z-10 bg-white/95 backdrop-blur-md text-teal px-[14px] py-[6px] rounded-full font-display text-[10px] font-bold uppercase tracking-widest shadow-sm">
                            {post.category}
                          </div>
                        ...
                        <div className="p-[28px] flex flex-col flex-grow bg-white">
                           ...
                           <div className="mt-auto flex items-center font-display text-[13px] font-medium text-teal mt-2">
                             Read article 
                 ...
                 {totalPages > 1 && (
                   <div className="flex items-center justify-center gap-2 mt-16">
                     <button 
                       ...
                       className="w-10 h-10 rounded-full flex items-center justify-center border border-gray-200 bg-white text-void hover:border-teal hover:text-teal hover:bg-ice disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                     ...
                     <div className="flex items-center gap-2 mx-2">
                       {Array.from({ length: totalPages }).map((_, i) => (
                         <button
                           ...
                           className={`w-10 h-10 rounded-full flex items-center justify-center font-display font-bold text-[14px] transition-all ${
                             currentPage === i + 1
             ...
             <div className="sticky top-[120px] flex flex-col gap-8">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    className="w-full pl-[56px] pr-[20px] py-[18px] rounded-full border border-gray-200 bg-white shadow-sm text-void font-body text-[15px] focus:border-teal focus:ring-4 focus:ring-teal/10 transition-all outline-none"
                  />
                </div>

                {/* Categories Filter */}
                <div className="bg-white rounded-3xl p-[32px] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100/60">
                   <h4 className="font-display text-[13px] font-bold tracking-[0.2em] uppercase text-void/80 mb-[24px]">
                     Topics
                   </h4>
                       ...
                       return (
                         <li key={cat}>
                           <button
                             ...
                             className={`w-full flex items-center justify-between px-[16px] py-[12px] rounded-xl transition-all group ${isActive ? 'bg-ice text-teal' : 'hover:bg-gray-50'}`}
                           >
                             <span className={`font-body text-[15px] transition-colors ${isActive ? 'font-semibold' : 'text-gray-600 group-hover:text-void'}`}>
                               {cat}
                             </span>
                             <span className={`font-display font-semibold text-[11px] px-[10px] py-[3px] rounded-full transition-colors ${isActive ? 'bg-teal/10 text-teal' : 'bg-gray-100 text-gray-500 group-hover:bg-gray-200'}`}>
                               {count}
                             </span>
                           </button>
                         </li>
                ...
                {/* Newsletter CTA */}
                <div className="bg-void p-[36px] rounded-3xl relative overflow-hidden shadow-xl">
                   {/* Background decoration */}
                   <div className="absolute top-0 right-0 w-32 h-32 bg-teal/30 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none" />
                   ...
                   <div className="relative z-10">
                     ...
                     <form onSubmit={handleSubscribe} className="flex flex-col gap-3">
                       <input 
                         ...
                         className="w-full bg-white/10 border border-white/20 text-white font-body text-[15px] px-[20px] py-[16px] rounded-full focus:border-cyan focus:bg-white/15 focus:ring-0 transition-all outline-none placeholder:text-white/40"
                       />
                       <button 
                         type="submit" 
                         disabled={isSubscribing}
                         className="w-full bg-lime text-void font-display font-extrabold text-[13px] uppercase tracking-widest px-[28px] py-[16px] rounded-full hover:bg-white hover:shadow-[0_0_20px_rgba(193,234,0,0.3)] transition-all flex items-center justify-center gap-2 group disabled:opacity-70 disabled:cursor-not-allowed"
                       >
